...
// ======= Secret (OpenAI) opcional (já estava) =======
...
function renderHTML(landing: any, slug: string): string {
  const nome = landing?.nome ?? slug;
  const modalidade = landing?.modalidade ?? "";
  const imagem = landing?.imagem ?? "";
  const plano = String(landing?.plano ?? "basic").toUpperCase();
  const cor = landing?.theme?.primary ?? "#0ea5e9";
  ...
  <section class="hero">
    ...
    <div class="card">
      ...
      <div class="cta" style="margin-top:16px">
        ${landing?.contatoEmail ? `<a href="mailto:${landing.contatoEmail}">Contato</a>` : ``}
      </div>
      ${landing?.bio ? `<p style="margin-top:16px; opacity:.9">${landing.bio}</p>` : ``}
    </div>
  </section>
