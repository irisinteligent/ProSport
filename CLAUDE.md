...
│   └── lib/
│       ├── firebase-admin.ts           # inicializa o Admin SDK; exporta `adminDb` e `adminAuth`; aceita credenciais via `FIREBASE_SERVICE_ACCOUNT` (Vercel) OU credenciais automáticas (App Hosting/Cloud Run)
...
| Geração de conteúdo (frontend) | Genkit (`genkit`, `@genkit-ai/googleai`) + Gemini (`gemini-1.5-flash-latest`) | ✅ Implementado, em `src/ai/flows/*` |
...
| 10 | `npm audit` com 3 vulnerabilidades críticas e 21 altas | Dependências desatualizadas (`fast-xml-parser`, `handlebars`, `protobufjs`, `next`, `axios`, `express`, etc.) com fixes não-major disponíveis, nunca aplicados | `npm audit fix` aplicado (sem `--force`) — reduziu para 0 críticas / 6 altas / 65 moderadas; as restantes exigem bump major de `@genkit-ai/*`/`firebase-admin` e foram deixadas para decisão separada (ver §10) | `package-lock.json` |
...
11. ~~`npm audit` com 100 vulnerabilidades (3 críticas, 23 altas)~~ — ⚠️ parcial: `npm audit fix` (sem `--force`) aplicado, restam 71 vulnerabilidades (0 críticas, 6 altas, 65 moderadas, ver §7 item 10). As restantes pedem bump major de `@genkit-ai/*`/`firebase-admin`/`firebase-functions-test` — avaliar separadamente antes de produção, pois pode quebrar os flows de IA.
